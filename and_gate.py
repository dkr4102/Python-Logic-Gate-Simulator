...
class ANDGateApp:
    def __init__(self, root):
        self.root = root
        self.root.title("AND GATE - Logic Gate Simulator")
        ...
        title = Label(header, text="AND GATE SIMULATOR", font=('Arial', 24, 'bold'),
                      bg="#0d1117", fg="#10b981")
        title.pack(side="left", expand=True)

        subtitle = Label(self.root, text="Formula: Y = A · B (Output is 1 ONLY when both inputs are 1)",
        ...
        # Output Y
        Label(ctrl_frame, text="Output Y:", font=('Arial', 14, 'bold'), bg="#161b22", fg="#10b981").grid(row=2, column=0, padx=10, pady=10, sticky="e")
        self.T3 = tk.Entry(ctrl_frame, font=('Arial', 16, 'bold'), width=5, justify='center', state='readonly', readonlybackground="#0d1117", fg="#10b981", relief="flat")
        ...
        calc_btn = tk.Button(btn_box, text="⚡ Calculate Output (Y = A · B)", font=('Arial', 13, 'bold'),
                             bg="#10b981", fg="#04210f", activebackground="#059669", activeforeground="#ffffff",
                             cursor="hand2", padx=20, pady=10, relief="flat", command=self.calculate_and)
    ...
    def load_schematic(self, parent):
        # Resolve path safely relative to script directory
        candidates = [
            os.path.join(os.path.dirname(__file__), "and_gate.png"),
            os.path.join(os.path.dirname(__file__), "assets", "and_gate.png")
        ]
        ...
        fallback = Label(parent, text="[ AND GATE SCHEMATIC: Y = A · B ]",
                         font=('Courier', 16, 'bold'), bg="#161b22", fg="#58a6ff", pady=30)
        fallback.pack()
    ...
    def calculate_and(self):
        ...
        if input1 in ['0', '1'] and input2 in ['0', '1']:
            ...
            if result == '1':
                self.status_lbl.config(text="Status: HIGH (True)", fg="#10b981")
            else:
